/**    
 *  This serverA.c file is as the serverA side.
 *  It could read the information of network from a file called serverA.txt.
...
#define SERVER_A_PORT_UDP "21133"
...
int main() {
	...
	*/
	...
	/*
		...
		And print the serverA itself TCP port number and the client TCP port number.
	*/
	...
	printf("The Server A finishes sending its neighbor information to the Client with TCP port number %s and IP address %s (Client's TCP port number and IP address).\n",CLIENT_PORT_TCP, LOCALHOSTADDR);
	...
	*/
	...
	socklen_t addr_len;
 	...
 	int status_udp;
   	...
   	if ((status_udp = getaddrinfo(NULL,SERVER_A_PORT_UDP, &hints_udp, &res_udp)) == -1) {
	...
	/*
		...
		Print the client UDP port number and the serverA itself UDP port number.
	...
	*/
    ...
    printf("Edge-Cost\n" );
 	printf("%s", topology);
 	printf("For this connection with Client, The ServerA has UDP port number %s and IP address %s\n",SERVER_A_PORT_UDP,LOCALHOSTADDR);
 	close(sockfd_udp);
	return 0;
}
