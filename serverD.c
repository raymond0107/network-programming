/**    
 *  This serverD.c file is as the serverC side.
 *  It could read the information of network from a file called serverD.txt.
...
#define SERVER_D_PORT_UDP "24133"
...
int main() {
	...
	*/
	...
	/*
		...
		And print the serverD itself TCP port number and the client TCP port number.
	*/
	...
	printf("The Server D finishes sending its neighbor information to the Client with TCP port number %s and IP address %s (Client's TCP port number and IP address).\n",CLIENT_PORT_TCP, LOCALHOSTADDR);
	...
	*/
	...
	socklen_t addr_len;
 	...
 	int status_udp;
   	...
   	if ((status_udp = getaddrinfo(NULL,SERVER_D_PORT_UDP, &hints_udp, &res_udp)) == -1) {
	...
	/*
		...
		Print the client UDP port number and the serverD itself UDP port number.
	...
	*/
    ...
    printf("Edge-Cost\n" );
 	printf("%s", topology);
 	printf("For this connection with Client, The ServerD has UDP port number %s and IP address %s\n",SERVER_D_PORT_UDP,LOCALHOSTADDR);
 	close(sockfd_udp);
	return 0;
}
